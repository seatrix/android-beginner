...
//			partition = list[i].substring(list[i].lastIndexOf("/") + 1);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		{			
			...
			filePath = "smb://";
			mSamba.startSearch(filePath, new OnSearchListenner() {
				@Override
		...
		for(String st:list){
			...
			if(file.isFile() && TypeFilter.isMovieFile(st)){
				...
				if(itg == null || pre.length() != 5){
					if(itg == null) Log.d(TAG, "----4---- null");
					if(pre.length() != 5) Log.d(TAG, "--------5----- no 5");
					continue;
				}
	...
	private void playPicture(String path)
	{
		...
		Intent picIntent = new Intent();
		//picIntent.setAction(android.content.Intent.ACTION_VIEW);
        ComponentName cm = new ComponentName("com.mipt.mediacenter", "com.mipt.mediacenter.picture.view.MainActivity");
        picIntent.setComponent(cm);       
